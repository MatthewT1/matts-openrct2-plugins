        ...
        for (const e of readdirSync(dir, { withFileTypes: true })) {
            ...
            if (e.isDirectory()) walk(p);
                ...
                for (const m of src.matchAll(/boolSetting\(storage, "(\w+)", (true|false)\)/g)) {
                    // src/trash/* belong to Trash Manager; otherwise the file's registerPlugin name.
                    const top = p.includes(join("src", "trash")) ? readFileSync(join("src", "trash-manager.ts"), "utf8") : src;
...
{
    const w = settingsToWrite("file.json", '{"Trash Manager":{"autoAmenities":true}}');
    ok(w["Trash Manager"].autoAmenities === true && Object.keys(w).length === 1, "json settings");
    let threw = false;
    try { settingsToWrite("f", '{"Trash Manager":{"nope":true}}'); } catch { threw = true; }
    ok(threw, "unknown key refused");
    threw = false;
    try { settingsToWrite("f", '{"Trash Manager":{"autoAmenities":"yes"}}'); } catch { threw = true; }
