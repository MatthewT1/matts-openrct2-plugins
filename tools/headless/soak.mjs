...
L.push("## Health and firing", "", "Deltas are summed over parks (end of run minus day 0). Runs are not exact repeats: scenario parks are seeded from the clock (Scenario.cpp:78), so treat single-park differences as noise.", "",
    "| Arm | runs ok | game-log errors | kiosks + | umbrella stalls + | queue TVs + | stalls + | entertainers + |", "|---|---|---|---|---|---|---|---|");
const health = {};
let prevErrs = null;
for (const arm of arms) {
    ...
    prevErrs = errs;
    L.push(`| ${arm} | ${ok.length}/${parks.length} | ${errs} | ${d("kiosks")} | ${d("umbrellaStalls")} | ${d("queueTvs")} | ${d("stalls")} | ${ent} |`);
}
