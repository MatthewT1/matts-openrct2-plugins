...
- Guest queue behavior: guests begin complaining after **5 minutes** of queuing and **leave the queue at 15 minutes** regardless of entertainment — documented wiki-sourced hardcoded thresholds. **Corrected (#68, source):** leaving needs queue timer >= 4300 **and** happiness <= 65, then a ~3% chance per check (`Guest.cpp:5729`); happy guests never leave.
...
- OpenRCT2 changed guest patience vs. vanilla RCT2: in vanilla, guests with entertainers/TVs nearby would wait indefinitely; in OpenRCT2 they still leave after ~15 minutes even with entertainers present — but only if unhappy (happiness <= 65, `Guest.cpp:5729`, #68). Strategies ported from vanilla-RCT2 guides assuming infinite patience are invalidated.
