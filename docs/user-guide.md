...
| Auto-build info kiosks, ATMs & umbrella stall | Auto-Builder | **ON** | **Yes** | Once the park has 3 rides: an ATM near the entrance, at the back and where guests run out of cash (not in no-money parks); an umbrella stall near the entrance (and the back of a big park); an information kiosk near the entrance, one at the far end of the paths, one at each side of a wide park, and one where guests say they're lost, unless one is already within 12 tiles; maps at 50p; at most 6 kiosks |
...
**Auto-place benches & bins where needed** (and **remove ones no longer needed**) — Costs money to build. Benches are cheap and stop guests vomiting near nauseating rides; bins catch litter near stalls. Removal only touches amenities this plugin placed, one a day at most, and only after it has looked unneeded for 10 days in a row. Since #117 it is a step more generous: up to 35 a day, a new one unless one is within 2 tiles, and blanket coverage every 5 tiles.
...
### Info Kiosks, ATMs and the Umbrella Stall: Front, Back and Where Guests Get Lost

Once the park has 3 real rides, Auto-Builder places up to four kinds of spot for an information kiosk, in this order: the park **entrance** (every guest passes it), the **back** of the park (the path tile the most steps from the entrance, only if that's at least 25 steps away), the **east and west sides** (the path tiles farthest to each side of the entrance-to-back line, only if at least 12 tiles off it), and the spot where the most guests are thinking "I'm lost" or "I can't find..." right now (3 or more). ATMs (if researched, and never in a no-money park) use the front, the back and the spot where the most guests think they are running out of cash. The umbrella stall (if researched) only uses the front and back, since no guest thought says "I want an umbrella". The entrance gets both before the back gets either. A spot that already has one of the same kind within 12 tiles, including one you built, is skipped. It builds at most one a day, needs £2,000 in the bank, and stops at 6 kiosks in the park. The map price is set to 50p.
