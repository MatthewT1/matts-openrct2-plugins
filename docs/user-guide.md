...
**Auto-place TVs on long queues** — Cheap path additions. A guest who has queued a long while loses happiness every moment unless their own queue tile has a TV, which lifts them instead, so TVs go on the front tiles where the longest waiters stand. Only on empty queue tiles (never replaces your bin, bench or lamp), and not on a tile joined on all four sides, which the game refuses.
