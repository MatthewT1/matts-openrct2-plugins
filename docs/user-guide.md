...
| Auto-build info kiosks & ATMs (front, back, clusters) | Auto-Builder | **ON** | **Yes** | Once the park has 3 rides: an information kiosk and an ATM near the entrance, one of each at the far end of the paths, a kiosk where guests say they're lost and an ATM where they're running out of cash, unless one is already within 12 tiles; maps at 50p; no ATMs in parks without money; at most 4 of each |
...
**Auto-build info kiosks & ATMs** — About £250 per kiosk and £200 per ATM, at most 4 of each in the park (yours count). A guest thinking "I'm running out of cash" walks to the nearest ATM and, if they're happy enough, tops up and keeps spending; there are no ATMs in a park without money, where nobody withdraws. A guest with a map plans further ahead, considers every ride in the park and checks the map when lost. Maps are set to 50p, below what guests think a map is worth, so nobody refuses one as too expensive. No waiting period: it's cheap safety, the way many players build by hand.
...
- An information kiosk and then an ATM appear near the entrance soon after the park has 3 rides (the console says "Built an information kiosk at the park front"), then one of each at the back of big parks
